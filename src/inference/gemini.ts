...
import { analyzeIntensity, computeSeriesPressure, countImminentWins } from './intensity';
import { computeMorale } from './morale';
...
function buildPrompt(
  ...
  moveNumber: number,
  series: SeriesState,
  lastRoundWinner: Mark | 'draw' | null = null,
): string {
...
async function geminiAnalyze(
  ...
  series: SeriesState,
  lastRoundWinner: Mark | 'draw' | null = null,
): Promise<{ intensity: number; morale_X?: number; morale_O?: number }> {
...
export async function getIntensity(
  ...
  try {
    ...
    if (
      typeof result.morale_X === 'number' && !isNaN(result.morale_X) &&
      ...
      morale = computeMorale(board, currentTurn, series, lastRoundWinner);
    }
    ...
    const morale = computeMorale(board, currentTurn, series, lastRoundWinner);
